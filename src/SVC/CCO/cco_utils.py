...
def ConfFileStage0(confFileName:str,
                   ...
                   **kwargs):
    """
    A basic config file format for the CCO. Default parameters can be changed by passing them as kwargs,
    e.g., by calling ConfFileStage0(confFileName, backboneFileName, outputFileName, thetaMin=0.3) to 
    change thetaMin to 0.3. 
    
    ### Parameters
    - confFileName: str
        The file where the configuration is saved.
    - backboneFileName: str
        The .cco file which is used as a backbone for the CCO algorithm. 
          Typically for this stage, this is the output of the SSM model.
    - outputFileName: str
        The file where the CCO tree will be saved (i.e., a .cco filename).
    - kwargs:
        Values of constraints for the CCO. Possible constraints are:
          - nTerm: int
                the number of terminal vessels to grow (default 100).
          - lLimFr: float
                Correction step factor (default 0.8)
          - gamma: float
                Power law coefficient in Murray's law (default 3.0)
          - delta: float
                Minimum symmetry ratio (default 0.8)
          - eta: float
                Viscosity in cP (default 0.36)
          - ViscosityTolerance: float
                Precision for the fixed point iteration scheme (default 1e-5)
          - thetaMin: float
                Minimum bifurcation angle, as a fraction of pi (default 0.3)
          - perfAreaFr: float
                Perfusion area factor (default 1.0)
          - closeNeighFr: float
                Close neighboirhood factor. Controls how far around a new 
                  terminal vessel the CCO looks for candidate vessels to connect to (default 1.0). 
    """
...
// Flow parameters, in muL/min & mmHg - to be converted to basic units --- Unused.
15.0
50.0
2.3 '''
...
def ConfFileStage1(confFileName:str,
    ...
    """
    A basic config file format for the CCO. Default parameters can be changed by passing them as kwargs,
    e.g., by calling ConfFileStage1(confFileName, backboneFileName, outputFileName, 
                                    nTerm0, nTerm1, thetaMin=0.3) 
    to change thetaMin to 0.3. 
    ...
    - outputFileName: str
        The file where the CCO tree will be saved (i.e., a .cco filename).
    - nTerm0, nTerm1: int
        Number of terminal vessels in the first/second sub-stage of growth.
    - kwargs:
        Values of constraints for the CCO. Possible constraints are:
          - lLimFr: float
                Correction step factor (default 0.8)
